...
class OldBeachline:
    par_count : dict[OldParabola, int]
    line : list[tuple[int, int, OldParabola]]  # from x1 inclusive to x2 exclusive lays par

    edges : list[OldEdge]

    _width : int
    _event_q : PriorityQueue
    _cur_y : int = -1

    def __init__(self, event_q : PriorityQueue, width : int):
        self.par_count = {}
        self.line = []
        self.edges = []
    ...
    def recalc_line(self, y : int):
        if len(self.line) < 2: return

        new_line : list[tuple[int, int, OldParabola]] = []

        for par in self.par_count.keys():
            if par.high_edges:
                xmin, xmax = self._width, -1
                for e in par.high_edges:
                    x1, x2 = e.get_range(y)
                    xmin = min(xmin, x1)
                    xmax = max(xmax, x2)
            else: xmin, xmax = 0, self._width

            if par.parasite_edges:
                t = []
                for e in par.parasite_edges: hq.heappush(t, e.get_range(y))

                if xmin - t[0][0]: hq.heappush(new_line, (xmin, t[0][0], par))
                if t[-1][1] - xmax: hq.heappush(new_line, (t[-1][1], xmax, par))
                for i in range(1, len(t)):
                    if t[i - 1][1] - t[i][0]: hq.heappush(new_line, (t[i - 1][1], t[i][0], par))
            else: hq.heappush(new_line, (xmin, xmax, par))

        self.line = new_line



    def new_raycast(self, site : Site):
        par = OldParabola(site)

        if self._cur_y < 0:
            self._cur_y = site.y()
            hq.heappush(self.line, (0, self._width, par))
            self.par_count[par] = 1
            return

        self.recalc_line(site.y())
        ...
        for i, e in enumerate(self.line):
            ...
            if x1 <= site.x() < x2:
                ...
                start_point = (x0, (p.get_point_int(y0, x0)))
                k = (x0 - p.x()) / (p.y() - y0)
                b = start_point[1] - x0 * k

                edge1 = OldEdge(start_point, y0, k, b, True, True)
                edge2 = OldEdge(start_point, y0, k, b, False, True)

                hq.heappush(self.edges, edge1)
                hq.heappush(self.edges, edge2)

                par.high_edges.append(edge1)
                par.high_edges.append(edge2)

                p.parasite_edges.append(edge1)
                p.parasite_edges.append(edge2)

                # self.line[i][1] = x0
                self.line[i] = (self.line[i][0], x0, self.line[i][2])
                hq.heappush(self.line, (x0, x0 + 1, par))
                hq.heappush(self.line, (x0 + 1, x2, p))
