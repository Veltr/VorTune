...
class OldEdge(FEntity):
    ...
    _last_range : tuple[int, tuple[int, int]]  # last y (x1, x2)

    def __init__(self, start_point : tuple[int, int], d_start : int, k : float, b : float, grow_right : bool, is_half : bool):
        self._start = start_point
        self._k = k
        self._b = b
        self._d0 = d_start
        self.grow_right = grow_right
        self.half_edge = is_half
        self._last_range = (-2, None)

    def x(self): return self._start[0]
    def y(self): return self._start[1]

    def get_range(self, d : int):
        if d == self._last_range[0]: return self._last_range[1]

        x0, y0 = self.x(), self.y()
        # tt = round(float(d - self._d0) / np.abs(self._k))
        tt = round(float(d - self._d0))

        if self.grow_right: r = x0, x0 + tt
        else: r = x0 - tt, x0

        self._last_range = (d, r)
        return r

    def get_points(self, d : int):
        x1, y1 = self.get_range(d)
        out = QPolygon()
        for i, x in enumerate(range(x1, y1)):
            t = round(self._k * x + self._b)
            out.append(QPoint(x, int(t)))
    ...
    def __lt__(self, other):
        ...
        return not self.grow_right

class OldParabola(FEntity):
    site : Site

    high_edges : list[OldEdge]
    parasite_edges : list[OldEdge]  # into parabola like parasite, ya know
